...
class TypeConverter {
    ...
    fun fromTimestampToLocalDate(value: Long?): LocalDate? {
        return value?.let {
            LocalDateTime.ofInstant(Instant.ofEpochMilli(value), ZoneOffset.UTC).toLocalDate()
        }
    }
    ...
    @TypeConverter
    fun stringToMap(value: String): HashMap<LocalDateTime, String> {
        val temp = Gson().fromJson(value, object : TypeToken<HashMap<Long, String>>() {}.type) as HashMap<Long, String>
        return temp.mapKeys {
            LocalDateTime.ofInstant(Instant.ofEpochMilli(it.key), ZoneOffset.UTC)
        } as HashMap<LocalDateTime, String>
    }

    @TypeConverter
    fun mapToString(value: Map<LocalDateTime, String>?): String {
        return if (value == null) "" else Gson().toJson(value.mapKeys {
            it.key.toInstant(ZoneOffset.UTC)?.toEpochMilli()
        })
    }
}